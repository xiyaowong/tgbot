...
func (p *WorkPool) workInit(id int) {
	//logger.Debug("start pool task worker id:%d", id)
	p.workNum++
	go func(idNum int) {
		defer func() {
			if err := recover(); err != nil {
				// stacks := tcp.PanicTrace(4)
				logger.Error("worker %d  exit panics: %v", idNum, err)
				p.workNum--
				p.workInit(idNum)
		...
		for {
			...
			case task := <-p.taskPool:
				if task.startBool == true && task.Run != nil {
					...
					//atomic.AddInt32(&p.tasks, -1)
					//logger.Emergency("Task Done By Worker Id:%d Cost Time:%s TotalworkersNum:%d", idNum, cost.String(), p.workNum)
				}
				//单个结束任务
