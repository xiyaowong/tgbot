...
func TGLuaVMRun(parms []interface{}) {
	...
	defer func() {
		...
		if err := recover(); err != nil {
			...
			logger.Error("TGLuaVMRun will exit panics: %v", err)
		}
	}()
	L.PreloadModule("mysql", mysql.Loader)
	L.PreloadModule("log", lualib.NewLogModule().Loader)
	L.PreloadModule("coreApi", NewCoreApiModule().Loader)
	L.PreloadModule("http", gluahttp.NewHttpModule(&http.Client{}).Loader)
	L.PreloadModule("json", lualib.NewJsonModule().Loader)
	...
	L.SetContext(ctx)
	defer cancel()
	scripts, _ := utils.GetLuaList("/Plugins/", ".lua")
	...
	for fName, scLua := range scripts {
		...
		if err != nil {
			logger.Error("TGLuaVMRun CallGlobal err %v detail %v", err, fName)
			return
		}
